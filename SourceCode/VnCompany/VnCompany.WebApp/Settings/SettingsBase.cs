...
namespace VnCompany.WebApp.Settings
{
    public abstract class SettingsBase
    {
        ...
        public virtual void Load(ISettingsService settingsService, bool useCache)
        {
            // ARGUMENT CHECKING SKIPPED FOR BREVITY
            // 3 get settings for this type name
            var settings = settingsService.LoadSettings(this._name, useCache).Result;
        ...
        public virtual void Save(ISettingsService settingsService)
        {
            // 5 load existing settings for this type            
            var settings = new List<IdentitySetting>();
            foreach (var propertyInfo in _properties)
            {
                ...
                var newSetting = new IdentitySetting()
                {
